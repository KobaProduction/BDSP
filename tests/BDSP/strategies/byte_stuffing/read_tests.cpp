...
TEST(byte_stuffing_read_strategy_tests, set_config) {
    ...
    read_strategy.init(nullptr, [](bool state, void *ctx) { *reinterpret_cast<bool *>(ctx) = state; }, &ready_state);
...
TEST(byte_stuffing_read_strategy_tests, error_read_tests) {
    ...
    ASSERT_TRUE(context.statuses ==
                std::vector<strategy_read_status_t>({STRATEGY_READ_ERROR, STRATEGY_READ_DELIMITER}));
}
